function ashleyNeuropixelResults(Animal)
% Animal = 'All';
% Animal = 'N14';
...
if strcmpi(Animal, 'all')
    fPath{1} = '\\grid-hs\churchland_nlsas_data\data\ashley_looming\PupilVideos\NP9\';
    fPath{2} = '\\grid-hs\churchland_nlsas_data\data\ashley_looming\PupilVideos\N14\';
...
%     depthClusters{1} = {[2541 3500], [1371 2540], [], [0 1370]};
%     depthClusters{2} = {[2311 3200], [2011 2310], [871 2010], [0 870]};
...
elseif strcmpi(Animal, 'N14')
    fPath{1} = '\\grid-hs\churchland_nlsas_data\data\ashley_looming\PupilVideos\N14\';
...
%% make R^2 single regressor figure
for z = 1:size(allV,4)-1
    meanRegs(1,z) = mean(cRegs(:, 1, z));
    meanRegs(2,z) = mean(cFull - cRegs(:, 2, z));
    
    semRegs(1,z) = sem(cRegs(:, 1, z));
    semRegs(2,z) = sem(cFull - cRegs(:, 2, z));
end

figure
errorbar(meanRegs(1,:),semRegs(1,:),'k-','linestyle','none','lineWidth',3); hold on
bar(meanRegs(1,:),'FaceColor','g','EdgeColor','k','BarWidth',0.5,'LineWidth',2);
errorbar(-meanRegs(2,:),semRegs(2,:),'k-','linestyle','none','lineWidth',3);
bar(-meanRegs(2,:),'FaceColor',[25 111 61]/255,'EdgeColor','k','BarWidth',0.5,'LineWidth',2);

ax = gca;
set(ax,'xTick',1:size(meanRegs,2))
set(ax,'xTickLabel',regGroups(1,1:size(meanRegs,2)))
set(ax,'XTickLabelRotation',45)
ax.TickLength = [0 0];
ylabel('cross-val. R^2'); ylim([-0.2 0.2]);
axis square; title('Single regressor R^2')
