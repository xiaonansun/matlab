function np_syncAnalysis


%% Additional analysis of the bit codes, doesn't need to be executed for the rest of the code to work

indDiv = [1 44 126 size(alignedSyncEventsBin,2)];
for i = 1:length(indDiv)-1
divSE{i}= alignedSyncEventsBin(:,indDiv(i):indDiv(i+1));
end
...
x = divSE{3};
idxBit = {[31:33]};
[matA,idxA] = np_syncSortBitEvents(x, idxBit);
imagesc([matA;x(~sum(idxA,2),:)]);

% print a string of bit codes for reading somewhere else
outString = np_syncGetStringOutput(matA);
%% Use a vertical line to visualize time
clear lineMask 
close all

colLine = 415:420;
lineMask = zeros(size(alignedSyncEvents,1),size(alignedSyncEvents,2));
lineMask(:,colLine) = ones(size(alignedSyncEvents,1),length(colLine));
imagesc(alignedSyncEvents + lineMask);

%%
colLine = 1110:1115;
endAlignedSyncEvents = align_sync_events_to_end(alignedSyncEvents);
lineMask = generate_line_mask(endAlignedSyncEvents,colLine);
...
% plot(smooth(sum(alignedSyncEvents),15))
