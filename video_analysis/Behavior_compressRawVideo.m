function Behavior_compressRawVideo(sPath, Animal, checkSVD)
...
% file. If checkSVD is true, conversion is only done when video folder
% contains the file motionSVD_CombinedSegments.mat (indicating that SVD has
...
if ~exist('checkSVD','var') || isempty(checkSVD)
    checkSVD = false;
end
...
for iRecs = 1 : length(recs)
    ...
    if checkSVD       
        checker = ~isempty(dir([sPath recs(iRecs).name filesep 'motionSVD_CombinedSegments.mat'])); %check if motionSVD_CombinedSegments.mat is present
    else
        checker = true;
    end
    
    if checker
        if strcmp(recs(iRecs).name, '.') || strcmp(recs(iRecs).name, '..') || ~isempty(strfind(recs(iRecs).name, 'mat'))
            rawVids = [];
        else
