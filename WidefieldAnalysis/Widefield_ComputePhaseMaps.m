...
%% Set basic variables
fclose('all');
path = 'Z:\data\WidefieldImager\Animals\';   %Widefield data path
...
opts.fName = 'Frames';
opts.fPath = [path Animal filesep 'PhaseMap' filesep Session];
opts.plotChans = true;
opts.trigLine = [6 7];
% opts.trigLine = NaN;
opts.blueHigh = true;
...
useVideo = false;
temp = ls([path Animal '\PhaseMap\' Session '\' opts.fName '*dat']);
temp1 = reshape(temp',1,numel(temp));
temp1 = strrep(temp1,'.dat','    ');
if isempty(temp1)
    temp = ls([path Animal '\PhaseMap\' Session '\' opts.fName '*' opts.fileExt]); 
    temp1 = reshape(temp',1,numel(temp));
...
for iTrials = Trials'
    ...
    if strcmpi(opts.fName,'acFrames')
        ...
        allData{iTrials} = Data; % added by RS
...
%% get vessel image.
cFile = ls([path Animal '\PhaseMap\' Session '\Snapshot_1.mat']); 
load([path Animal '\PhaseMap\' Session '\' cFile]);
...
% save([path Animal '\PhaseMap\' Session '\allData.mat'],'allData');
% save([path Animal '\PhaseMap\' Session '\avgData.mat'],'avgData');
