...
temp = ls([path Animal '\' Paradigm '\' Session '\Frames*mj2']);
...
for iTrials = Trials'
    ...
    [blueData,blueTimes,hemoData,hemoTimes,stimOn,falseAlign,sRate] = Widefield_SplitVideo(opts,iTrials);
    
    Data = Widefield_HemoCorrect(blueData,hemoData,1:ceil(opts.preStim * sRate),5); %hemodynamic correction
    
    cFile = [path Animal '\' Paradigm '\' Session '\Frames_' num2str(iTrials) '.dat']; %current file to be read
    ...
    load([path Animal '\' Paradigm '\' Session '\TriggerTime_' num2str(iTrials) '.mat']); %load recording onset times
    ...
    firstFrame = triggerTime(1) + dSwitch; %timePoint of stimulus onset, based on trigger signal.
