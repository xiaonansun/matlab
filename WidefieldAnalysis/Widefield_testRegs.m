function Widefield_testRegs(cPath,Animal,Rec,nonOrth)
...
if sum(fullR(:,1)) == size(fullR,1)
    ...
    recLabels = recLabels(2:end); %remove offset if present
    recIdx = recIdx(2:end) - 1; %remove offset if present
    idx = idx(2:end); %remove offset if present
end
