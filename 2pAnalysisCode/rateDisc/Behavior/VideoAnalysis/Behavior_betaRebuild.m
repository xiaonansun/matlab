function Behavior_betaRebuild(cPath,fileExt)
...
load([cPath fileExt 'regData.mat'],'recIdx', 'regLabels', 'rejIdx','redQRR'); %load R from QR decomposition
...
if undoQR
    ...
    vidIdx = ismember(recIdx(~rejIdx), find(ismember(regLabels,{'bhvVideo'}))); % index for video regressors
    vidB = dimBeta(vidIdx, :);
    ...
    betas = dimBeta(recIdx(~rejIdx) == find(strcmp(regLabels, 'bhvVideo')), :)';
end
