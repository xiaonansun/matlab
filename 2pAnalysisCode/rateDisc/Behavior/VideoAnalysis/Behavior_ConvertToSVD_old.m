function Behavior_ConvertToSVD(cPath,iCam)
opts.fPath = cPath;
opts.fName = 'Video';
opts.trigLine = NaN;
...
%% get trial numbers for all video files from current camera
recs = dir([opts.fPath filesep '*' opts.fName '*' num2str(iCam) '.mj2']);
for iRecs = 1:length(recs)
    a = textscan(recs(iRecs).name,'%s%s%s%s%s%s%f%f%s','Delimiter','_');
    trials(iRecs) = a{end-2};
end
trials = sort(trials);
...
mSM30_SpatialDisc_May30_2017_Session3_Video_0001_1.mj2

for iTrials = 1:length(trials) %check individual datasets for framecount
...
ind = frameCnt < prctile(frameCnt,90); % only use trials that have a complete framecount.
trials(ind) = []; frameCnt(ind) = [];
ind = frameCnt ~= median(frameCnt); %only use trials that have the same framecount.
trials(ind) = []; frameCnt(ind) = [];

cFile = [opts.fPath filesep 'mask.mat'];
if ~(exist(cFile,'file') == 2) %check if mask exists already
    blueData = Widefield_CheckChannels(opts,trials(1));
...
[blueData,~,hemoData] = Widefield_CheckChannels(opts,trials(1),blueHigh);
blueData = single(squeeze(blueData));
...
for iTrials = 1:length(trials)
    ...
    [blueData,blueTimes,hemoData,hemoTimes] = Widefield_CheckChannels(opts,trials(iTrials),blueHigh);
...
recs = dir([opts.fPath filesep 'alignedBlueFrames_*']);
for iRecs = 1:length(recs)
...
for iTrials = 1:length(trials)        
    ...
    hemoTimes = header(1:end-length(size(hemoData))) * (86400*1e3); %extract frame times from header and convert to milliseconds
    hemoData = bsxfun(@minus, single(hemoData), hemoAvg); % subtract mean
    hemoData = bsxfun(@rdivide, hemoData, hemoAvg); % divide mean
    hemoData = arrayShrink(hemoData,mask,'merge'); %only use selected pixels from mask
    delete(cFile); %delete aligned data file
