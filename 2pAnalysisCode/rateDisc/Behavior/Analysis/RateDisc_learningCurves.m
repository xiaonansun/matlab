...
[params, ~, stats] = Behavior_fitPalamedes((1 : length(detectCurve)) ./ length(detectCurve) , detectCurve.*nTrials, nTrials, false, true);
...
fitRange = (fitRange - fitRange(1)) ./ (fitRange(end) - fitRange(1)) .* (length(detectCurve) - 2) + 1; %rescale fitrange back to sessionNrs
learnRange = params.guessRate + (1 - params.lapseRate - params.guessRate) * prcRange/100; % get 5, 50 and 95th percentile
learnLocs = interp1(cFit, fitRange, learnRange); %find location of percentiles on the curve

figure; %make figure for estimates
plot(detectCurve(2 : end), 'o', 'linewidth', 2, 'MarkerSize', 8); hold on; ax = gca;
...
ax.XLim = [1 length(detectCurve)-1]; axis square; 
ax.YLim = [0.4 1];
...
figure; %make figure for estimates
plot(discCurve(2 : end), 'o', 'linewidth', 2, 'MarkerSize', 8); hold on; ax = gca;
plot(fitRange, cFit, 'linewidth', 4); 
...
cDates = Performance.Date(1,tactileDiscOn : tactileDiscOff); %dates fpr detection sessions
