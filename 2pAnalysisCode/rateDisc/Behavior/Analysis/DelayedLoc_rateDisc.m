function [Performance,bhv] = DelayedLoc_rateDisc(Animal,cPath,lSessions,binSize,highDetection,showPlot,minDelay)
...
[cDate,ind] = sort(cDate,'ascend'); %sort counts to get the order of files to days correct. Newest file should be first in the list.
cDate = floor(cDate);
...
for iFiles = 1:size(Files,1)
    ...
    if useData
        ...
        Performance.fileName{Cnt} = Files(iFiles,:);
        ...
        for iMod = 1:3
            ...
            if ~isnan(Performance.Discrimination(iMod,Cnt))
                ...
                for iTrials = find(ind)
                    tCnt = tCnt +1;
                    [left, right] = Behavior_getStimEvent(SessionData.StimType(iTrials), SessionData.stimEvents{iTrials});
                ...
                [params, ~, ~, cFit] = Behavior_fitPalamedes(distRatio, rightChoice, nTrials); %complete discrimination parameters
...
if Cnt > 0
    ...
    sessionSelect(any(lowInd)) = []; %don't use low performance sessions of selected
