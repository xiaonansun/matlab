function rateDisc_globalChoiceDecode(animal, stepWin, decType, cMod, trialsPerBin)
...
sRate = 30;
ndims = 200; %both wV and newU must have enoug components for this to work
...
if stepWin <= iSteps
    ...
    Vc = cFile.wV(1:ndims, :, cIdx(1)+1:cIdx(2)); %data for current sessions
    Vc = rateDisc_getBhvRealignment(Vc, cBhv, segFrames, opts); %aligned to different trial episodes
    [cvAcc, bMaps, trialCntWin] = rateDisc_logDecoder(Vc, U, cBhv, trialsPerBin, cMod, regType, decType);
    fprintf('Decoder finished\n')
    toc;

    if ~exist([bPath 'decodeSegments'], 'dir'); mkdir([bPath 'decodeSegments']); end
    save([bPath 'decodeSegments' filesep modTitle '_' decType '_seg_' num2str(stepWin, '%04i')], 'cvAcc', 'bMaps', 'trialCntWin', 'cIdx', 'allTrials', 'recs','-v7.3')
