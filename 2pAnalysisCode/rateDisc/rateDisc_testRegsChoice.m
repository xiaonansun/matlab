function rateDisc_testRegsChoice(cPath,Animal,Rec,fileExt,cRegs)
...
extraGroups = {'handles' 'licks' 'Choice' 'handleChoice' 'stimChoice' 'respChoice' ...
               'audioStim' 'video' 'labelMove' 'motor' 'sensory' 'cognitive'};
extraGroups{2,1} = {'lGrab' 'rGrab'};
extraGroups{2,2} = {'lLick' 'rLick' 'lick'};
extraGroups{2,3} = {'lhandleChoice' 'rhandleChoice' 'lstimChoice' 'rstimChoice' ...
                    'lresponseChoice' 'rresponseChoice'}; %all choice regessors
extraGroups{2,4} = {'lhandleChoice' 'rhandleChoice'}; %handle choice regessors
extraGroups{2,5} = {'lstimChoice' 'rstimChoice'}; %stim choice regessors
extraGroups{2,6} = {'lresponseChoice' 'rresponseChoice'}; %response choice regessors
extraGroups{2,7} = {'lfirstAudStim' 'rfirstAudStim' 'lAudStim' 'rAudStim' 'handleSound'}; %all auditory stimulus regessors
extraGroups{2,8} = {'bhvVideo' 'Move'}; %all video regressors
extraGroups{2,9} = motorLabels(~ismember(motorLabels,{'bhvVideo' 'Move'})); %non-video movements
extraGroups{2,10} = motorLabels; %all motor regressors
extraGroups{2,11} = sensorLabels;
extraGroups{2,12} = cogLabels;
...
oMotorLabels = ['handles' motorLabels(~ismember(motorLabels, extraGroups{2, ismember(extraGroups(1,:),'handles')}))];
oMotorLabels = ['licks' oMotorLabels(~ismember(oMotorLabels, extraGroups{2, ismember(extraGroups(1,:),'licks')}))];
oMotorLabels = ['video' oMotorLabels(~ismember(oMotorLabels, extraGroups{2, ismember(extraGroups(1,:),'video')}))];
