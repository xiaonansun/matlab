function bhv = rateDisc_loadDiscriminationBhv(Animals, cPath, newRun)
...
if ~newRun
    try
        load([cPath 'rateDisc' filesep 'optoDisc_' Animals{:}], 'bhv')
    catch ME
        disp(ME.message);
        newRun = true;
...
if newRun
    ...
    for x = 1 : length(bhv.stimEvents)
        bhv.stimEvents{x} = cellfun(@single, bhv.stimEvents{x}, 'UniformOutput', false); %single conversion to reduce size a bit
    end
    bhv = rmfield(bhv,'TrialSettings');
    save([cPath 'rateDisc' filesep 'optoDisc_' Animals{:}], 'bhv', '-v7.3')
end
