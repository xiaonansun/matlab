...
for iAnimals = 1 : length(animals)
    ...
    for iRecs = 1 : length(recs)
        ...
        fPath = [cPath cAnimal filesep 'SpatialDisc' filesep recs(iRecs).name filesep]; %path to current dataset
        try
            % get some data
            load([fPath 'orgdimBeta.mat'])
            load([fPath 'orgregData.mat'], 'regIdx', 'rejIdx', 'regLabels')
            load([fPath 'Vc.mat'], 'U')
            ...
            U = arrayShrink(U, allenMask);
            allU(:,:,Cnt) = U; clear U
            
            % get dimensions for different betas
            for iRegs = unique(regIdx)
                allRegs{Cnt, iRegs} = regLabels{iRegs};
                betaData{Cnt, iRegs} = NaN(sum(regIdx == iRegs), size(dimBeta,2), 'single'); %pre-allocate for all weights
                betaData{Cnt, iRegs}(~rejIdx(regIdx == iRegs),:) = dimBeta(regIdx(~rejIdx) == iRegs, :);  %pick up used regressor weights
            end
...
%             vidBeta = dimBeta(recIdx(~idx) == find(strcmpi(regLabels, 'bhvVideo')), :); %video weights
%             [~,~,c] = pca(vidBeta);
...
vidLabels = {'Choice' 'lfirstAudStim' 'rfirstAudStim' 'prevChoice' 'prevStim' 'nextChoice' 'repeatChoice'};
betaVideos = NaN(size(allenMask,1), size(allenMask,2), videoLength, length(vidLabels), 'single');

for iRegs = 1 : length(regLabels)
    for iAnimals = 1 : Cnt
        
        if ismember(regLabels{iRegs}, vidLabels)
            cLength = size(betaData{iAnimals, iRegs},1);
            if ismember(regLabels{iRegs},{'nose'})
                cRange = ceil((cLength-1)/2) + 2 : cLength; %high amp events
            elseif ismember(regLabels{iRegs},{'whisk'})
            ...
            a = betaVideos(:,:,1:length(cRange),ismember(vidLabels, regLabels{iRegs})) .* (iAnimals-1);
            b = arrayShrink(allU(:,:,iAnimals) * smoothCol(betaData{iAnimals, iRegs}(cRange,:))', allenMask, 'split');
...
%             end
            ...
            c(isnan(c)) = a(isnan(c));
            c(isnan(c)) = b(isnan(c));
            betaVideos(:,:,1:length(cRange),ismember(vidLabels, regLabels{iRegs})) = c ./ iAnimals;
            clear a b c
        end
    end
end
...
for iRows = 1 : length(rowLabels)
    for iMaps = 1 : length(rowLabels{iRows})
        cMap = betaMaps(:, :, strcmp(regLabels, rowLabels{iRows}{iMaps}));
