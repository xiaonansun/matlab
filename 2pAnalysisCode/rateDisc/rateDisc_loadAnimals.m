function bhv = rateDisc_loadAnimals(Animals, cPath, newRun)
...
if ~newRun
    try
        load([cPath 'rateDisc' filesep 'optoBhv_' Animals{:}], 'bhv')
    catch ME
        disp(ME.message);
        newRun = true;
...
if newRun
    ...
    save([cPath 'rateDisc' filesep 'optoBhv_' Animals{:}], 'bhv', '-v7.3')
end
...
fiberLocations = { 'Frontal' 'Parietal' 'S1'};
fiberColors = {[0 0 1] [1 0 0] [0.5 0.5 0]};
...
h = figure('Renderer','openGL'); hold on
for x = 2 : -1 : 1
...
dOut.optoDiff = rateDisc_stimANDdelayOptoDiff(bhv); %out.detect: early/late x ctrl/opto x location x animals
