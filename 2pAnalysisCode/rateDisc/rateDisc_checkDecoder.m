...
savePath = 'Q:\BpodImager\logDecoder\'; %save path for clustering
rightHs = find(ismember(dorsalMaps.sidesSplit,'R')); %index for labels on the left HS
stepSize = 3; %steps are binned together. At 15Hz, stepSize 3 == 200ms
expDsize = floor(85 / stepSize); %expected size of data array. should be 85 frames per trial at 15Hz, divided by stepSize.
reload = false;
...
try
    if reload
        ...
        load([savePath 'allACC.mat'],'allACC');
        load([savePath 'vidACC.mat'],'vidACC');
        load([savePath 'allBeta.mat'],'allBeta','animals');
    end
catch
    vidACC = cell(1, length(animals));
    allACC = cell(1, length(animals));
    allBeta = NaN(sum(~allenMask(:)), expDsize, 3, 6, length(animals), 'single');
    for iAnimals = 1 : length(animals)
        ...
        vidACC{iAnimals} = NaN(expDsize, 4, length(recs), 'single');
        allACC{iAnimals} = NaN(expDsize, 3, 6, length(recs), 'single');
        cBeta = NaN(sum(~allenMask(:)),expDsize, 3, 6, length(recs), 'single');
        %go through recordings
        for iRecs = 1 : length(recs)
            
            try
                fPath = [cPath cAnimal filesep 'SpatialDisc' filesep recs(iRecs).name filesep]; %Widefield data path
                load([fPath 'logDecode_lasso_' int2str(stepSize) '_0.mat'], 'cvACC', 'cvBeta', 'trialCnt')
                load([fPath 'logNoMotorDecode_lasso_' int2str(stepSize) '_0.mat'], 'nmACC', 'nmBeta')
                load([fPath 'logNoSpontMotorDecode_lasso_' int2str(stepSize) '_0.mat'], 'nsmACC', 'nsmBeta')
                load([fPath 'logVideoDecode_lasso_' int2str(stepSize) '_0.mat'], 'vidChoice')
                load([fPath 'opts2'], 'opts');
                
                % get mask
                if exist([fPath 'rsVc.mat'], 'file')
                    mFile = matfile([fPath 'rsVc.mat']);
                else
                    mFile = matfile([fPath 'Vc.mat']);
                end
                mask = isnan(mFile.U(:,:,1)); clear mFile
                
                % choice predictor performance
                allACC{iAnimals}(:,1,:,iRecs) = cvACC;
                allACC{iAnimals}(:,2,:,iRecs) = nmACC;
                allACC{iAnimals}(:,3,:,iRecs) = nsmACC;
                vidACC{iAnimals}(:,:,iRecs) = vidChoice;
                
                %maps
                cvBeta = arrayShrink(cvBeta, mask, 'split');
                cvBeta = alignAllenTransIm(cvBeta,opts.transParams);
                cvBeta = rateDisc_removeOutline(cvBeta,5);
                cBeta(:, :, 1, :, iRecs) =  arrayShrink(cvBeta,allenMask, 'merge');
                
                nmBeta = arrayShrink(nmBeta, mask, 'split');
                nmBeta = alignAllenTransIm(nmBeta,opts.transParams);
                nmBeta = rateDisc_removeOutline(nmBeta,5);
                cBeta(:, :, 2, :, iRecs) =  arrayShrink(nmBeta,allenMask, 'merge');
                
                nsmBeta = arrayShrink(nsmBeta, mask, 'split');
                nsmBeta = alignAllenTransIm(nsmBeta,opts.transParams);
                nsmBeta = rateDisc_removeOutline(nsmBeta,5);
                cBeta(:, :, 3, :, iRecs) =  arrayShrink(nsmBeta,allenMask, 'merge');
                
                fprintf('Recording %d/%d\n', iRecs, length(recs));
            catch ME
                fprintf('Failed loading recording %d/%d\n', iRecs, length(recs));
                disp(ME.message);
            end
        end
        cBeta = nanmean(cBeta, 5); %average maps over recordings
        allBeta(:, :, :, :, iAnimals) = cBeta;
    end
    save([savePath 'allACC.mat'],'allACC','-v7.3');
    save([savePath 'vidACC.mat'],'vidACC','-v7.3');
    save([savePath 'allBeta.mat'],'allBeta','animals','-v7.3');
end
