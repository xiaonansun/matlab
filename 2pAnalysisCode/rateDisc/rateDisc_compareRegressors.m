% function rateDisc_compareRegressors(cMod,reload)
...
%% select data sets
[dataOverview, motorLabels, sensorLabels, cogLabels, ~, segLabels] = rateDiscRecordings;
...
sPath = 'R:\smusall\predVariance\'; %local data path to save down results
...
animals = animals(contains(animals, cMod)); %subselect animals
...
%% make venn diagram to show overlap with video and time regressors
fullModel = double(aRsq(strcmpi(fullRecLabels,'full'), cInd, ismember(modTypes,'shCurrent'))); %this is the full model.
timeModel = double(aRsq(strcmpi(fullRecLabels,'time'), cInd, ismember(modTypes,'shOther'))); %this is the current regressor model. All regressor information.
timeUnique = double(fullModel - aRsq(strcmpi(fullRecLabels,'time'), cInd, ismember(modTypes,'shCurrent')));  %this is the current regressor. This has non-redundant information.
videoModel = double(aRsq(strcmpi(fullRecLabels,'allMove'), cInd, ismember(modTypes,'shOther'))); %this is the current regressor model. All regressor information.
videoUnique = double(fullModel - aRsq(strcmpi(fullRecLabels,'allMove'), cInd, ismember(modTypes,'shCurrent')));  %this is the current regressor. This has non-redundant information.

figure;
venn([nanmean(fullModel), nanmean(timeModel), nanmean(videoModel)], ...
    [nanmean(timeModel)-nanmean(timeUnique) nanmean(videoModel)-nanmean(videoUnique) nanmean(timeModel)/3 nanmean(timeModel)/3]);
...
% figure
% [~, idxGroup] = regressorPlot(squeeze(cData(1,idx>0,:))',fullRecLabels(idx>0),5,subplot(2,2,1:2),[0 1 0],idx(idx>0),0.6, 0.4);
% title('All (top) / Unique (bottom)'); xlim([0 30]); %make this a fixed value to ensure bars have always the same width
% ylabel('total cv R^2');
% ax = regressorPlot(squeeze(cData(2,idx>0,:))',fullRecLabels(idx>0),5,subplot(2,2,3:4),[25 111 61]/255,idxGroup,0.6, 0.4);
% xlim([0 30]); %make this a fixed value to ensure bars have always the same width
% set(ax,'Ydir','reverse')
% ylabel('unique delta cv R^2');
...
%% Task-dependent R2 reduction for motor regressors
fullM = aRsq(ismember(fullRecLabels,{'full'}), cInd, 1); %full model
taskM = aRsq(ismember(fullRecLabels,{'motor'}), :, ismember(modTypes,'shCurrent')); %this is the task
motorM = aRsq(ismember(fullRecLabels,{'motor'}), :, ismember(modTypes,'shOther')); %this is the motor
opTaskM = aRsq(ismember(fullRecLabels,{'opMotor'}), :, ismember(modTypes,'shOtherMotor')); %this is the task + operant movement model.
spTaskM = aRsq(ismember(fullRecLabels,{'spontMotor'}), :, ismember(modTypes,'shOtherMotor')); %this is the task + spontaneous movement model.
opM = aRsq(ismember(fullRecLabels,{'opMotor'}), :, ismember(modTypes,'shOther')); %this is the operant movement model.
spM = aRsq(ismember(fullRecLabels,{'spontMotor'}), :, ismember(modTypes,'shOther')); %this is the operant movement model.

spTaskAllShare = spM - (spTaskM - taskM);
spTaskShare = (fullM - opM) - (fullM - motorM) - (fullM - opTaskM);
spTaskOpShare = spTaskAllShare - spTaskShare;

opTaskAllShare = opM - (opTaskM - taskM);
opTaskShare = (fullM - spM) - (fullM - motorM) - (fullM - spTaskM);
spTaskOpShare2 = opTaskAllShare - opTaskShare;
motorTaskAllShare = motorM - (fullM - taskM);

cData = [mean(opTaskShare) mean(spTaskShare) (mean(motorTaskAllShare)-mean(opTaskShare)-mean(spTaskShare))];
cError = [sem(opTaskShare) sem(spTaskShare) sem((motorTaskAllShare)-(opTaskShare)-(spTaskShare))];

figure;
errorbar(cData,cError,'k-','linestyle','none','lineWidth',2); hold on
bar([mean(opTaskShare) mean(spTaskShare) (mean(motorTaskAllShare)-mean(opTaskShare)-mean(spTaskShare))],'FaceColor','b','EdgeColor','k','BarWidth',0.6,'LineWidth',2)
axis square;
title('Unique task overlap - opMotor, spontMotor, allMotor');
ylabel('pred. R^2');
...
for x = 1:length(otherMotorLabels)
        ...
        cRegTaskOp = aRsq(ismember(fullRecLabels,otherMotorLabels{x}), :, ismember(modTypes,'shOtherSpontMotor')); %this is the current regressor + operant movement + task.
        cRegOp = aRsq(ismember(fullRecLabels,otherMotorLabels{x}), :, ismember(modTypes,'shTaskOtherSpontMotor')); %this is the current regressor + operant movement
        
        uniqueTask = (cRegTaskOp - cRegOp);
        uniqueRegTask = (cRegTaskOp - opM);
        uniqueReg = (cRegTaskOp - opTaskM);
        regTaskShare = uniqueRegTask - uniqueTask - uniqueReg; %unique shared variance between regressor and task, without op movement overlap
        
        
        cRegMod = aRsq(ismember(fullRecLabels,otherMotorLabels{x}), :, ismember(modTypes,'shOther')); %this is the current regressor model. All regressor information.

        
        cData(1,Cnt,:) = cRegMod ; %subtract unique information from full model. Remaining is task-redundant information.
        cData(2,Cnt,:) = regTaskShare;
...
figure
[ax, allInd] = regressorPlot(squeeze(cData(1,idx>0,:))',otherMotorLabels(idx>0),5,subplot(2,2,1:4),[0,191,255]/255,idx(idx>0),0.6);
ax = regressorPlot(squeeze(-cData(2,idx>0,:))',otherMotorLabels(idx>0),5,ax,[0,0,139]/255,allInd,0.6,[-0.4 0.4]);
xlim([0 30]);
title('Regressor comparison - Task-dependancy (independent of OpMotor variables in dark blue)');
ylabel('crossVal R^2');
xlim([0 30]);