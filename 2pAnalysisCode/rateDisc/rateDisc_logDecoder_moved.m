function [cvAcc, bMaps, betaNeuron, mdlAll, trialCnt, allAUC, shufAUC] = rateDisc_logDecoder(dataIn, U, bhv, useTrials, targMod, regType, stepSize, decType, learnType, shuffleAUC)
    ...
    if strcmpi(decType, 'allChoice')
        ...
        if sum(choiceIdx) >= useTrials
            ...
            if ~isempty(U)
                ...
                allAUC(useIdx, Cnt) = colAUC(X',Y);
                if shuffleAUC == 1
                    for iShuf = 1:nShuf
                        shufAUC(useIdx, Cnt, iShuf) = colAUC(X',Y(randperm(length(Y))));
                    end
                end
            end
        end
