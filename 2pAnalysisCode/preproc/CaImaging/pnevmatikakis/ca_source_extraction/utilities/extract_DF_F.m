function [C_df,Df] = extract_DF_F(Y,A,C,ind,options,AYprovided)
...
if ~exist('AYprovided', 'var') % FN added, so we don't need Y (huge variable) as an input anymore. % if 1, Y (the 1st input) will be in fact AY.
    AYprovided = 0;
end
...
if AYprovided % FN added, so we don't need Y (huge variable) as an input anymore.
    AY = Y; clear Y
    AY = spdiags(nA,0,K,K)\AY; % FN: we need to normalize it just how A gets normalized here.
else
    step = 5e3;
    if memmaped
        AY = zeros(K,T);
