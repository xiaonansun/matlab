...
% init_test
if isfield(options, 'normingMedianImage')
    if ssub~=1
        normMed = imresize(options.normingMedianImage, [ds(1), ds(2)], 'box');
    else
        normMed = options.normingMedianImage;    
    end
    
    % normalizing Y_ds with median image
     Y_ds = bsxfun(@rdivide,Y_ds,normMed);
end
