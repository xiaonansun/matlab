function imData=bigread2_frs2use(path_to_file, framesToUse) % sframe,num2read) %FN
...
% ofds=zeros(numFrames, 1); % FN added 1.
ofds=zeros(length(framesToUse), 1); % FN added 1.
f = 0;
for i=framesToUse; % 1:numFrames % FN
    f  = f+1;
    ofds(f)=info(i).StripOffsets(1);
    %ofds(i)
end
% FN
...
fseek(fp, ofds(1), 'bof');
%framenum=numFrames;
framenum=length(framesToUse); % num2read; %FN
...
if strcmpi(form,'uint16') || strcmpi(form,'uint8')
    if(bo)
        for cnt = 1:length(framesToUse)  % sframe:lastframe %FN
            %cnt;
