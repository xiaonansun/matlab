...
%% Load data
load(iDataPath,'frames_per_trial','reg_group_name','reg_idx','X','Y');
%%

% Vc is the neural data
% fullR is the design matrix
rejIdx = false(size(reg_idx,1),size(reg_idx,2));
regLabels = cellstr(reg_group_name);
[ridgeVals, dimBeta] = ridgeMML(X, Y, true); % ALL REGRESSORS: get ridge penalties and beta weights
