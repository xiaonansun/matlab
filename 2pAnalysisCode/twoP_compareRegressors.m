function twoP_compareRegressors(cMod, reload, noMove)

if ~exist('reload','var') || isempty(reload)
    reload = false;
end

if ~exist('noMove','var') || isempty(noMove) || ~noMove
    noMove = false;
    fileExt = [];
else
    fileExt = 'noMove';
...
cPath = 'U:\smusall\2pData\Animals\';
...
sPath = 'C:\2pData\predVariance\'; %local data path to save down results
...
if ~reload %check for a saved dataset
    if ~exist(sPath,'dir')
        ...
        try
            load([sPath fileExt 'corrMaps.mat'],'corrMaps');
            load([sPath fileExt 'meanRsq.mat'],'aRsq','aTimeRsq','nRecLabels');
            load([sPath fileExt 'segMovies.mat'],'segMovies');
            load([sPath fileExt 'fullCorrMaps.mat'],'fullCorrMaps', 'recSite', 'recDepth', 'recAnimal', 'recDate');
            load([sPath fileExt 'fullSegMovies.mat'],'fullSegMovies');
            load([sPath fileExt 'otherMsegMovies.mat'],'otherMsegMovies');
            load([sPath fileExt 'otherMcorrMaps.mat'],'otherMcorrMaps');
            load([sPath fileExt 'twoP_rebuild.mat'],'recV', 'semV', 'allBeta', 'recLabels', 'allRecIdx', 'dataPath', 'modIdx', 'sideIdx', 'alignIdx', 'baseLength', 'frames', 'stimTimes', 'cellCnt', 'otherMotorLabels');
...
%             disp(fPath); 
    ...
    save([sPath fileExt 'fullCorrMaps.mat'],'fullCorrMaps', 'recSite', 'recDepth', 'recDate', 'recAnimal');
    save([sPath fileExt 'fullSegMovies.mat'],'fullSegMovies');
    save([sPath fileExt 'corrMaps.mat'],'corrMaps');
    save([sPath fileExt 'segMovies.mat'],'segMovies', '-v7.3');
    save([sPath fileExt 'otherMsegMovies.mat'],'otherMsegMovies', '-v7.3');
    save([sPath fileExt 'otherMcorrMaps.mat'],'otherMcorrMaps');
    save([sPath fileExt 'meanRsq.mat'],'aRsq','aTimeRsq','nRecLabels');
    save([sPath fileExt 'twoP_rebuild.mat'],'recV', 'semV', 'allBeta', 'recLabels', 'allRecIdx', 'dataPath', 'modIdx', 'sideIdx', 'alignIdx', 'baseLength', 'frames', 'stimTimes', 'cellCnt', 'otherMotorLabels', '-v7.3');
end
...
% figure; hold on;
% plot(fullCorrMaps(cellIdx),'k'); axis square;
% plot(-fullCorrMaps(cellIdx),'k'); axis square;
% plot(motorRed(cellIdx),'b'); axis square;
% plot(motorFull(cellIdx),'c'); axis square;
% plot(-taskFull(cellIdx),'m'); axis square;
% plot(-taskRed(cellIdx),'r'); axis square;
% hline(0);
% ylabel('crossVal R^2');
% xlabel('Neurons')
% xlim([0 length(cellIdx)]);
...
%     allExp = (cat(1, recV(cIdx & ~baseRejIdx & recExp == 'v', :, 4, end-3), recV(cIdx & ~baseRejIdx & recExp == 'a', :, 5, end)));
%     allNov = (cat(1, recV(cIdx & ~baseRejIdx & recExp == 'a', :, 4, end-3), recV(cIdx & ~baseRejIdx & recExp == 'v', :, 5, end)));
%     
%     stdshade(allExp(:, 1 : baseLength), 'k', (1 : baseLength) / 31, 0.5); hold on
%     stdshade(allExp(:, cStim : end), 'k', (cStim : frames) / 31, 0.5); hold on
% 
%     stdshade(allNov(:, 1 : baseLength), 'r', (1 : baseLength) / 31, 0.5); hold on
%     stdshade(allNov(:, cStim : end), 'r', (cStim : frames) / 31, 0.5); hold on
%     title([areas{iAreas} ' - ' num2str(sum(cIdx & ~baseRejIdx))]); axis square
end
...
%% get tuning of neurons in different areas/depths
% areas = unique(recSite); %different recording sites
% depths = [0 350 1000]; %define range of depths to group neurons
% for iRecs = 1 : size(recV,4)
%     temp = std(recV(:, :, 6, iRecs), [], 2);
%     
%     for iAreas = 1 : length(areas)
%         for iDepths = 1 : length(depths) - 1
%             
%             cIdx = ismember(recSite,areas{iAreas}) & recDepth > depths(iDepths) & recDepth < depths(iDepths + 1);
%             taskMod{iAreas,iDepths}(:, iRecs) = temp(cIdx);
%             taskModCnt(iAreas, iDepths, iRecs) = sum(temp(cIdx) > std(temp)) ./ sum(cIdx);
%             
%         end
%     end
% end
...
%     cCell = 11972; %instructed cell1
    ...
    title('task');
%     
%     delete(subplot(2,8,5:8));
%     ax2 = subplot(2,8,5:8);
%     cLabels = recLabels(ismember(recLabels,motorLabels));
%     [a, b] = sort(squeeze(sum(abs(recV(cCell,:,avgType,ismember(recLabels,motorLabels))),2))', 'descend');
%     bar(ax2,double(a(1:5))./totalVar);
%     set(ax2,'xTick',1:5)
%     set(ax2,'xTickLabel',cLabels(b(1:5)))
