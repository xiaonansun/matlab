function [Vm, cBeta, cR, subIdx, cRidge, cLabels, cMap] =  crossValModel_unnested(cLabels,regLabels,regIdx,rejIdx,Vc,fullR,ridgeFolds)

% OLD FUNDTION: function [Vm, cBeta, cR, subIdx, cRidge, cLabels, cMap, cMovie] =  crossValModel(cLabels)
if ~exist('rejIdx','var')
    rejIdx = false(size(regIdx,1),size(regIdx,2));
elseif isempty(rejIdx)
    rejIdx = false(size(regIdx,1),size(regIdx,2));
end

dType = 'twoP';
if ~exist('ridgeFolds','var')
    ridgeFolds = 10;
end
